...
Follow that prose with a simple bullet list. Each top-level bullet is one sequential implementation step and one future commit. Label each bullet with its step number and a short title. Nested bullets may explain scope, rationale, dependencies on earlier or later steps, expected files, and validation. Avoid deeply nested bullets.
...
Then provide a concise summary with a bullet for each changed file. Refer to each file by path and explain what changed and why. Also report:
...
4. Verify that the commit contains the intended files and that no step-related changes were left behind.
